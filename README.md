...
1. **팀원**
    ...
    - 프로젝트 아키텍처 설계
    - UI/UX 설계
2. 🎯메인 페이지 REST API 서버 구현 **) SpringBoot, React**
    - 관리자 페이지
        - 커뮤니티 사용자 신고내역 관리 기능 구현
        - 플래그 컬럼을 통한 유저 정보 **소프트 삭제(soft delete) 기능 구현**
        - 유저 리스트 검색 및 페이징 처리 기능 구현
    - 메인 페이지
        - 메인, 로그인 페이지 UI/UX 설계
        - 메인 페이지 구성 리스트 페이징 처리
        - FrontEnd 측에서 사용자 권한을 기준으로 한 예외처리 수행
3. 🧑🏻‍💼**개발 외 담당 업무**
    - CS 지식을 주제로 팀원 간 **스터디 참여**
